...
interface Lesson { dayIdx: number; hourId: number; subjectId: string; teacherId: string; roomId: string; theme: string; changeKind: ChangeKind; changeDesc: string; beginTime: string; endTime: string; }
interface Hour   { Id: number; Caption?: string; BeginTime: string; EndTime: string; }
...
export default function Timetable() {
  ...
  const getWeekDate = (o: number) => { const d = new Date(); const dow = d.getDay()||7; d.setDate(d.getDate()-dow+1+o*7); return d.toISOString().slice(0,10); };
  ...
  const getDayDate = (i: number) => { const d = ttData?.Days?.[i]?.Date; if (!d) return ''; const x = new Date(d); return `${x.getDate()}. ${x.getMonth()+1}.`; };
  const getWeekRange = () => { if (!ttData?.Days?.length) return ''; const fmt = (d: string) => { const x = new Date(d); return `${x.getDate()}. ${x.getMonth()+1}.`; }; try { return `${fmt(ttData.Days[0].Date)} – ${fmt(ttData.Days[ttData.Days.length-1].Date)}`; } catch { return ''; } };

  useEffect(() => {
    setLoading(true); setLessons([]); setHours([]);
      ...
      if (data) {
        ...
        (data.Days || []).forEach((day: any, di: number) => {
          (day.Atoms || []).forEach((atom: any) => {
            ...
            out.push({ dayIdx: di, hourId: atom.HourId, subjectId: atom.SubjectId, teacherId: atom.TeacherId, roomId: atom.RoomId, theme: atom.Theme||'', changeKind: ck, changeDesc: atom.Change?.Description||atom.Change?.TypeName||'', beginTime: hr.BeginTime, endTime: hr.EndTime });
          });
        });
        setLessons(out);
      }
  ...
  const Cell = ({ l }: { l: Lesson }) => {
    ...
    return (
      <div className={cn('rounded-lg border p-2 h-full min-h-[72px] flex flex-col gap-0.5 text-[10px]', c.bg, c.border)}>
        {l.changeKind !== 'normal' && (
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex bg-[#18181b] border border-[#27272a] rounded-lg p-1">
            <button onClick={() => setWeekOffset(o => o-1)} className="p-1.5 hover:bg-[#27272a] rounded text-[#a1a1aa] hover:text-[#fafafa] transition-colors"><ChevronLeft size={15}/></button>
            <div className="px-3 flex items-center text-xs font-medium text-[#fafafa] min-w-[120px] justify-center">{weekLabel}</div>
            <button onClick={() => setWeekOffset(o => o+1)} className="p-1.5 hover:bg-[#27272a] rounded text-[#a1a1aa] hover:text-[#fafafa] transition-colors"><ChevronRight size={15}/></button>
          </div>
        </div>
      </div>
          ...
          <div className="md:hidden space-y-4">
              ...
              {DAYS.map((d, i) => {
                ...
                return (
                  <button key={d} onClick={() => setActiveDay(i)}
                    className={cn('flex flex-col items-center px-3 py-2 rounded-lg border transition-all min-w-[56px] relative shrink-0',
            ...
            <div className="space-y-2">
              {hours.filter(hr => cell(activeDay, hr.Id)).map((hr, i) => {
                ...
                const r = rm(l.roomId);
                return (
                  <motion.div key={hr.Id} initial={{opacity:0,y:6}} animate={{opacity:1,y:0}} transition={{delay:i*0.04}}
                    className={cn('glass-card p-3 flex items-center gap-3 border-l-[3px]', c.bg, c.border,
                      l.changeKind==='canceled' ? 'border-l-rose-500 opacity-75' :
                      l.changeKind==='substitution' ? 'border-l-amber-400' :
                      l.changeKind==='other' ? 'border-l-purple-400' : 'border-l-indigo-500/50')}>
                    {/* Time */}
                    ...
                    <div className="flex-1 min-w-0">
                      {l.changeKind !== 'normal' && (
                        <span className={cn('text-[9px] font-bold uppercase px-1.5 py-0.5 rounded inline-block mb-1', c.badge)}>{c.label}</span>
                      )}
